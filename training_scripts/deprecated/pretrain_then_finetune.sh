...
max_seq_len=2048
checkpoint_dir="${MANIFOLD_DIR}/pythia-wsd/pythia-1b-wsd"
pretraining_data_dir="${FINEWEB_DIR}"
instruction_data_json="${MANIFOLD_DIR}/all_in_one_pretraining/datasets/sft_reasoning_full/concat/"
run_id=${EPOCHSECONDS}
is_on_tc=false
lr_scheduler="cosine"
...
litgpt pretrain $model_name \
  ...
  --data.val_data_path /data/datasets/hf_cache/data/fineweb/sample-350BT/val/0 \
  --data.num_workers $SLURM_GPUS_ON_NODE \
